...
Favicon / apple-touch / Open Graph files are generated by
`scripts/generate-seo-assets.py` into `src/app/` (`favicon.ico`, `icon.png`,
`apple-icon.png`, `opengraph-image.jpg`). Until the custom domain is attached,
`metadataBase` uses the Vercel production host so `og:image` actually resolves.
