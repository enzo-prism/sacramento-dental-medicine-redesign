    ...
    <main id="main" className="flex-1">
      ...
      <section className="section bg-wash" aria-labelledby="booking-title">
        <div className="container-x grid gap-10 md:grid-cols-2">
          ...
          <div><h2 className="font-display text-3xl font-semibold text-ink">Where are you, and when are you open?</h2><address className="mt-5 not-italic leading-8 text-ink-soft">{contact.practiceName}<br />{contact.addressLine1}<br />{contact.addressLine2}</address><div className="mt-3 flex flex-wrap gap-5"><a href={contact.emailHref} className="btn-text">{contact.email}</a><a href={contact.mapsHref} className="btn-text">Get directions to the Antelope office</a></div><dl className="mt-6 space-y-2 text-sm text-ink-soft">{hours.map(({ day, time }) => <div key={day} className="flex justify-between gap-4"><dt>{day}</dt><dd>{time}</dd></div>)}</dl><p className="mt-4 text-sm leading-7 text-ink-soft">All hours are Pacific time. Wednesday and Thursday appointments extend to 7 PM.</p></div>
        </div>
      </section>
    </main>
    <Footer /><MobileCTA />
